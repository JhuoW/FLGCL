...
if __name__ == '__main__':
    ...
    preprocess_path = osp(args.data_path, args.dataset, args.mode)
    if not osp.exists(preprocess_path):
        databuild = DataBuild(args, dataset_config['ratio_train'], dataset_config['seed'])
        if args.mode == 'disjoint':
            for n_clients in dataset_config['clients']:  
