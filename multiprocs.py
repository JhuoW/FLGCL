...
from models.client import Client as ClientClass
from models.server import Server as ServerClass
import time
import sys
...
class ParentProcecss:
    def __init__(self, args, config, Server, Client):
        ...
        self.server: ServerClass = Server(args, config, self.gpu_server, self.sd)
    ...
    def start(self):
        ...
        self.num_clients = round(self.args.n_clients + self.args.frac)
        ...
        for curr_comm in range(self.config['n_comm']):  
            ...
            # 随机排列所有client id
            self.selected = np.random.choice(self.args.n_clients, self.num_clients, replace=False).tolist()
            # 将当前server中的global model参数设置到 sd['global']['model']中 用于在server-client间共享
            self.server.on_communication_begin(self.selected)  
...
class WorkerProcess:
    def __init__(self, args, config, worker_id, gpu_id, q, sd, Client: ClientClass): 
        '''
        q: 表示
        '''
        self.args = args 
