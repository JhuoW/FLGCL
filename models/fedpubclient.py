...
from models.fedpubnets import * 
from misc.utils import *
import time

class Client(ClientModule):

    def __init__(self, args, config, w_id, g_id, sd):
        super(Client, self).__init__(args, config, w_id, g_id, sd)
        self.model = MaskedGCN(config['num_feats'], 128, config['num_cls'], 0.001, self.args).cuda(g_id) 
